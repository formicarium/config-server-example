...
(defn expose-service
  [world]
  (assoc world :service-fn (get-in world [:system :config-server-example-api :service ::http/service-fn])))

(defn init!
  [world]
  (-> world
      (assoc :system (component/start-system (config-server-example/system-map :test)))
      expose-service))
